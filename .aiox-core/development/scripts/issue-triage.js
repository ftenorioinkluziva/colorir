...
function applyLabels(number, type, priority, areas, extra) {
	...
	gh(
		`issue edit ${number} --add-label ${addStr} --remove-label "status: needs-triage"`,
	);
	console.log("  Done.");
}
...
function generateReport() {
	...
	console.log("\nBy Area:");
	for (const [k, v] of Object.entries(stats.byArea).sort(
		(a, b) => b[1] - a[1],
	)) {
		console.log(`  ${k}: ${v}`);
	}
	console.log("\nBy Status:");
	for (const [k, v] of Object.entries(stats.byStatus).sort(
		(a, b) => b[1] - a[1],
	)) {
		console.log(`  ${k}: ${v}`);
	}
...
if (args.includes("--list")) {
	...
	const number = Number.parseInt(args[idx + 1]);
	...
	console.log("Usage:");
	console.log(
		"  --list                                    List untriaged issues",
	);
	console.log(
		"  --apply <#> --type <t> --priority <P> --area <a>  Apply labels",
	);
	console.log(
		"  --report                                  Generate triage summary",
	);
