...
class GreetingBuilder {
	...
	buildFixedLevelGreeting(agent, level) {
		...
		switch (level) {
			case "minimal":
				greetingText =
					profile.greeting_levels.minimal || `${fallbackIdLabel} Agent ready`;
				break;
			case "named":
				greetingText =
					profile.greeting_levels.named || `${fallbackLabel} ready`;
				break;
			case "archetypal":
				greetingText =
	...
	buildSimpleGreeting(agent) {
		...
		const greeting =
			greetingLevels?.named || `${this._formatAgentLabel(agent)} ready`;
		return `${greeting}\n\nType \`*help\` to see available commands.`;
	}
