...
const INDEX_RESOLUTION_EXTENSIONS = [
	".js",
	".mjs",
	".ts",
	".yaml",
...
function resolveEntityId(
	...
	while (
		existingEntities[candidate] &&
		existingEntities[candidate].path !== relPath
	) {
...
function resolveRelativeDependencyId(refPath, currentFilePath) {
	if (
		!currentFilePath ||
		!(refPath.startsWith(".") || refPath.startsWith("/"))
	) {
		return null;
	}
...
function looksLikePlaceholder(candidate) {
	...
	const interpolationLength = interpolationMatches.reduce(
		(sum, m) => sum + m.length,
		0,
	);
	if (interpolationLength > 0 && interpolationLength / s.length > 0.3)
		return true;
...
function extractPurpose(content, filePath) {
	...
	if (frontmatterMatch) {
		...
		const fmDescMatch = fm.match(
			/^(?:description|purpose|summary)\s*:\s*(.+)$/im,
		);
		if (fmDescMatch) {
