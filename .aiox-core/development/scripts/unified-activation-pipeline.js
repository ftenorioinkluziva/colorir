...
class UnifiedActivationPipeline {
	...
	async _runPipeline(
		...
		const tier1Budget = this._resolveLoaderTimeout(
			"critical",
			LOADER_TIERS.critical.timeout,
		);
		...
		const tier2Budget = this._resolveLoaderTimeout(
			"high",
			LOADER_TIERS.high.timeout,
		);
		...
		const tier3Budget = this._resolveLoaderTimeout(
			"bestEffort",
			LOADER_TIERS.bestEffort.timeout,
		);
