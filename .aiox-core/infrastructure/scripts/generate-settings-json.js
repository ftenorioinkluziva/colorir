...
function expandSubdirWithExceptions(subdirPath, exceptions, projectRoot) {
	...
	for (const entry of entries) {
		...
		const isCoveredByException = exceptions.some(
			(exc) => exc === entryGlob || exc === entryRelative,
		);
	...
	for (const globPath of protectedPaths) {
		if (globPath === ".aiox-core/core/**") {
			...
			for (const dir of dirs) {
				const relevantExceptions = exceptions.filter((exc) =>
					isChildOf(exc, dir),
				);
...
function writeSettingsJson(projectRoot, permissions) {
	...
	if (generatedPermissionsEmpty) {
		...
		const mergedAllow = Array.from(
			new Set([
				...(Array.isArray(existingPermissions.allow)
		...
		const mergedDeny = Array.from(
			new Set([
				...(Array.isArray(existingPermissions.deny)
