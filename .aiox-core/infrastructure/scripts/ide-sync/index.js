...
const kimiTransformer = require(
	path.resolve(__dirname, "transformers", "kimi"),
);
...
function transformPrimaryContent(transformer, agent, ideName) {
	if (
		ideName === "claude-code" &&
		typeof transformer.transformCommand === "function"
	) {
...
function syncIde(agents, ideConfig, ideName, projectRoot, options) {
	const result = {
		...
		skillRootDir: ideConfig.skillsPath
			? path.join(projectRoot, ideConfig.skillsPath)
			: null,
		files: [],
		skillFiles: [],
		errors: [],
	...
	for (const agent of agents) {
		...
		try {
			...
			if (ideConfig.format === "kimi-skill" && transformer.getDirname) {
				...
				if (
					!isPathInside(targetRoot, skillDir) ||
					!isPathInside(targetRoot, targetPath)
				) {
