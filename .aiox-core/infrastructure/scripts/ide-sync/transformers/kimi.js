...
function transform(agentData) {
	...
	const whenToUse =
		rawAgent.whenToUse ||
		fallbackAgent.whenToUse ||
		`Use for ${title.toLowerCase()} tasks`;
...
function buildProtocolSection(yaml) {
	...
	if (yaml.evidence_policy) {
		...
		if (ep.required_min_sources)
			text += `- Required minimum sources: ${ep.required_min_sources}\n`;
	...
	if (yaml.confidence_model) {
		...
		if (cm.thresholds) {
			text += "- Thresholds:\n";
			for (const [k, v] of Object.entries(cm.thresholds))
				text += `  - ${k}: ${v}\n`;
		}
		items.push(text + "\n");
	}
...
function buildCommandsTable(commands) {
	...
	if (!hasGuide)
		allRows.push("| `*guide` | Show comprehensive usage guide | full |");
...
function normalizeCommand(cmd) {
	...
	return {
		name,
		description:
			value === null || value === undefined ? "No description" : String(value),
		visibility: ["full"],
	};
}
...
function buildWorkflowSection(yaml) {
	...
	if (yaml.mandatory_flow && Array.isArray(yaml.mandatory_flow)) {
		items.push(
			"### Mandatory Flow\n\nExecute **strictly in this order**. No skips allowed.\n\n" +
				yaml.mandatory_flow
					.map((step, i) => `${i + 1}. ${renderItem(step)}`)
					.join("\n") +
				"\n",
		);
	}
	...
	if (
		yaml.activation_instructions &&
		Array.isArray(yaml.activation_instructions)
	) {
		items.push(
...
function buildHandoffsSection(yaml) {
	if (
		!yaml.handoffs ||
		!Array.isArray(yaml.handoffs) ||
		yaml.handoffs.length === 0
	) {
		return "";
	}
...
function getPreferredActivationId(agentData) {
	...
	const preferred =
		agent.preferredActivationAlias || agent.preferred_activation_alias;
