...
function sanitizeEvent(event) {
	return {
		...
		invocation_count: Math.max(
			0,
			Math.floor(Number(event.invocation_count) || 0),
		),
		token_cost_input: Math.max(
			0,
			Math.floor(Number(event.token_cost_input) || 0),
		),
		token_cost_output: Math.max(
			0,
			Math.floor(Number(event.token_cost_output) || 0),
		),
...
function validateEvent(event) {
	...
	if (
		typeof event.invocation_count !== "number" ||
		event.invocation_count < 0
	) {
		return {
			valid: false,
			errors: ["invocation_count must be a non-negative number"],
		};
	}
...
async function main() {
	...
	const sessionId =
		sessionIdIdx >= 0 && args[sessionIdIdx + 1]
			? args[sessionIdIdx + 1]
			: generateSessionId();
	...
	if (sampleMode) {
		...
		if (stdinData && Array.isArray(stdinData)) {
			...
			if (!jsonOutput) {
				...
				console.log(
					"       node collect-tool-usage.js --sample --session-id my-session",
				);
	...
	const validationResults = sanitized.map((e) => ({
		event: e,
		...validateEvent(e),
	}));
	...
	if (invalid.length > 0 && !jsonOutput) {
		console.warn(
			`[TOK-5] Warning: ${invalid.length} events failed validation and were skipped.`,
		);
		invalid.forEach((r) =>
			console.warn(`  - ${r.event.tool_name}: ${r.errors.join(", ")}`),
		);
	}
	...
	const session = {
		...
		total_token_cost_input: valid.reduce(
			(sum, e) => sum + e.token_cost_input,
			0,
		),
		total_token_cost_output: valid.reduce(
			(sum, e) => sum + e.token_cost_output,
			0,
		),
		events: valid,
	};
	...
	const result = {
		...
		total_tokens:
			session.total_token_cost_input + session.total_token_cost_output,
