...
function listClaudeAgentSkillIds(skillsAgentsDir) {
	...
	return fs
		...
		.filter(
			(entry) =>
				entry.isDirectory() &&
				fs.existsSync(path.join(skillsAgentsDir, entry.name, "SKILL.md")),
		)
...
function validateClaudeIntegration(options = {}) {
	...
	const agentsDir =
		options.agentsDir ||
		path.join(projectRoot, ".claude", "commands", "AIOX", "agents");
	const skillsAgentsDir =
		options.skillsAgentsDir ||
		path.join(projectRoot, ".claude", "skills", "AIOX", "agents");
	const hooksDir =
	...
	if (!fs.existsSync(agentsDir)) {
		warnings.push(
			`Missing legacy Claude commands dir: ${path.relative(projectRoot, agentsDir)}`,
		);
	}
	if (!fs.existsSync(skillsAgentsDir)) {
		errors.push(
			`Missing Claude agent skills dir: ${path.relative(projectRoot, skillsAgentsDir)}`,
		);
	}
	...
	for (const sourceAgent of sourceAgents) {
		...
		if (!skillContent.includes("activation_type: pipeline")) {
			errors.push(
				`Claude agent skill missing activation_type: pipeline: ${sourceAgent}`,
			);
		}
	}
