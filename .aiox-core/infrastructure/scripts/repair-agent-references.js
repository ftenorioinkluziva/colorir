...
async function repairAgentReferences(options = {}) {
	...
	const geminiValidation = validateGeminiCommands(
		agents,
		resolved.geminiCommandsDir,
	);

	return {
		ok:
			localValidation.ok &&
			(globalValidation ? globalValidation.ok : true) &&
			geminiValidation.ok,
...
function formatHumanReport(result) {
	...
	const detailErrors = [
		...
		...(result.validations.globalCodex
			? result.validations.globalCodex.errors
			: []),
