...
function validateSkillContent(content, expected) {
	...
	const requiredChecks = [
		...
		{
			ok: content.includes(
				`.aiox-core/development/agents/${expected.filename}`,
			),
 ...
 */
function createSkillToolSelfTestPayload(
	skillId,
	prompt = "AIOX skill self-test",
) {
	return {
		type: "tool_use",
		name: "Skill",
	...
	for (const item of resolved.expected) {
		...
		try {
			...
			results.push({
				...
				ok: false,
				errors: [
					`self-test unable to read ${relativeSkillPath} (${error.message})`,
				],
			});
			continue;
		}
		...
		if (frontmatter.error) {
			...
			if (declaredSkillId !== item.skillId) {
				errors.push(
					`self-test frontmatter name mismatch: expected "${item.skillId}"`,
				);
			} else {
		...
		if (!canonicalAgentPath) {
			...
			const absoluteAgentPath = path.join(
...
function getThinLegacyRedirectIssues(content, canonicalSkillId, legacySkillId) {
	...
	const allowedLinePatterns = [
		...
		new RegExp(`^#\\s+${legacy}$`),
		new RegExp(
			`^This legacy alias redirects to canonical skill \`${canonical}\`\\.$`,
		),
	...
	if (
		!lines.some((line) =>
			/^<!--\s*AIOX-CODEX-LEGACY-ALIAS:\s*redirect\s*-->$/.test(line),
		)
	) {
	...
	if (
		...
		issues.push(
			`missing exact canonical redirect sentence for "${canonicalSkillId}"`,
		);
	}
...
function isIntentionalLegacyAlias(content, canonicalSkillId, legacySkillId) {
	return (
		getThinLegacyRedirectIssues(content, canonicalSkillId, legacySkillId)
			.length === 0
	);
}
...
function classifyLegacySkillAlias(content, item, relativeSkillPath) {
	...
	if (hasLegacyAliasIntent(content, item.skillId)) {
		const issues = getThinLegacyRedirectIssues(
			content,
			item.skillId,
			item.legacySkillId,
		);
...
function validateCodexSkills(options = {}) {
	...
	const expectedByLegacyId = new Map(
		expected.map((item) => [item.legacySkillId, item]),
	);
	...
	if (resolved.strict) {
		const dirs = fs
			...
			.filter(
				(entry) =>
					entry.isDirectory() &&
					(entry.name.startsWith("aiox-") || entry.name.startsWith("aios-")),
			)
