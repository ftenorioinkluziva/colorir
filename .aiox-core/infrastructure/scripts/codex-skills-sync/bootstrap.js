...
function listFiles(dirPath, predicate) {
	...
	return fs
		...
		.filter((filePath) => {
			try {
				return (
					fs.statSync(filePath).isFile() && (!predicate || predicate(filePath))
				);
			} catch (_error) {
				return false;
			}
...
function parseAgentFile(filePath, yaml) {
	...
	const name =
		String(agent.name || "").trim() || findScalar(yamlBlock, ["name"]) || title;
	const whenToUse =
	...
	const commands = collectCommands(parsed.commands).concat(
		collectCommandsFromText(yamlBlock),
	);
...
function resolveEntryAgent(configData, squadDir) {
	...
	const preferred = agentFiles.find((filePath) =>
		/(^|-)chief$/i.test(basenameNoExt(filePath)),
	);
	if (preferred) return slug(basenameNoExt(preferred));

	const anyChief = agentFiles.find((filePath) =>
		/chief/i.test(basenameNoExt(filePath)),
	);
...
function resolveAgentFile(squadDir, configData, entryAgent) {
	...
	if (Array.isArray(parsed.agents)) {
		const match = parsed.agents.find(
			(agent) => agent && slug(agent.id) === entryAgent,
		);
	...
	return (
		agentFiles.find(
			(filePath) => slug(basenameNoExt(filePath)) === entryAgent,
		) || ""
	);
}
...
function squadSkillId(squadName, entryAgent, squadAlias) {
	...
	const genericEntryRoots = new Set([
		"agent",
		"chief",
		"master",
		"orchestrator",
	...
	if (entry === squadBase || entry.startsWith(`${squadBase}-`))
		return `aiox-${entry}`;
	if (alias && (entry === alias || entry.startsWith(`${alias}-`)))
		return `aiox-${entry}`;
	if (
		entryRoot &&
		!genericEntryRoots.has(entryRoot) &&
		squadBase.startsWith(`${entryRoot}-`)
	) {
		return `aiox-${entry}`;
	}
	if (
...
function buildSquadPlans(projectRoot, yaml) {
	...
	for (const squadDir of squadDirs) {
		...
		if (!sourceFile) {
			warnings.push(
				`Skipped ${squadName}: entry/chief file not found for ${entryAgent}`,
			);
			continue;
		}
...
function writePlanItem(item, skillsDir, options) {
	...
	if (options.dryRun) {
		return {
			status: fs.existsSync(filePath) ? "would_update" : "would_create",
			path: rel,
		};
	}
	...
	if (fs.existsSync(filePath)) {
		...
		return {
			status: backup ? "updated_with_backup" : "updated",
			path: rel,
			backup,
		};
	}
...
function dedupePlans(plans) {
	...
	for (const item of plans) {
		...
		warnings.push(
			`Renamed duplicate skill ${item.skillId} to ${replacement.skillId}`,
		);
...
function main() {
	...
	if (!args.quiet) {
		...
		console.log(
			"- Use the aiox-* skill names to avoid matching unrelated global aios-* skills.",
		);
		console.log(
			"- This script does not delete or modify global skills in ~/.codex/skills.",
		);
	}
}
