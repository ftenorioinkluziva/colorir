...
const NON_PROVIDER_CONFIG_KEYS = new Set([
	"ai_providers",
	"model_switching",
	"parallel_execution",
]);
...
function getProvider(providerName, config = null) {
	...
	const providerType = normalizeProviderName(
		providerConfig.provider || normalizedName,
	);
...
async function getAvailableProviders() {
	const providers = getConfiguredProviderNames().map((name) =>
		getProvider(name),
	);
...
function isProviderConfigKey(key, value) {
	if (
		NON_PROVIDER_CONFIG_KEYS.has(key) ||
		!value ||
		typeof value !== "object"
	) {
		return false;
