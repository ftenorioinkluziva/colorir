...
class OpenAICompatibleProvider extends AIProvider {
	...
	constructor(config = {}) {
		...
		this.endpoint = this._normalizeEndpoint(
	...
	async execute(prompt, options = {}) {
		...
		if (typeof this.fetchFn !== "function") {
			throw new Error(
				`[${this.name}] fetch API is not available in this runtime`,
			);
		}
		...
		if (!model) {
			throw new Error(
				`[${this.name}] model is required for OpenAI-compatible execution`,
			);
		}
	...
	_buildPayload(prompt, model, options = {}) {
		...
		this._copyDefined(
			payload,
			"thinking",
			options.thinking ?? this.options.thinking,
		);
		this._copyDefined(
			payload,
			"response_format",
	...
	_extractOutput(responseBody) {
		...
		const content =
			choice?.message?.content ?? choice?.text ?? responseBody?.text ?? "";
