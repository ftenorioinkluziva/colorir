...
function classifyError(err) {
	...
	if (code === "AUTH_RATE_LIMITED" || code === "RATE_LIMITED") {
		const retry =
			err.details && err.details.retryAfter
				? ` (retry em ${err.details.retryAfter}s)`
				: "";
		return {
			exitCode: 2,
...
async function validateAction(options) {
	...
	if (!isValidEmail(email)) {
		if (asJson) {
			process.stdout.write(
				`${JSON.stringify({ error: "INVALID_EMAIL", email })}\n`,
			);
		} else {
...
async function validateBatchAction(options) {
	...
	const concurrencyRaw =
		options && options.concurrency ? Number(options.concurrency) : 5;
	...
	if (!filePath || !fs.existsSync(filePath)) {
		const msg = filePath
			...
			: "Erro: --file é obrigatório.";
		if (asJson) {
			process.stdout.write(
				`${JSON.stringify({ error: "FILE_NOT_FOUND", file: filePath })}\n`,
			);
		} else {
	...
	if (asJson) {
		...
		for (const r of results) {
			if (r.error) {
				...
				process.stdout.write(
					`${icon}  ${r.email}  buyer=${r.isBuyer} account=${r.hasAccount}\n`,
				);
			}
		}
	...
	if (options.json) {
		process.stdout.write(
			JSON.stringify({
				...
				story:
					"docs/stories/epic-123/STORY-123.8-cohort-buyer-cli-migration.md",
			}) + "\n",
		);
	} else {
		process.stderr.write(
...
function createBuyerCommand() {
	...
	cmd
		...
		.option(
			"-c, --concurrency <n>",
			"Requisições paralelas (default 5, máx 10)",
			"5",
		)
