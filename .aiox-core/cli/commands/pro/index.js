		...
		if (fs.existsSync(proSourceDir)) {
			...
			if (scaffoldProContent) {
				...
				if (scaffoldResult.success) {
					...
					console.error(
						'Try running "aiox pro activate" again to retry scaffolding.',
					);
				}
				console.log("");
			}
...
async function updateAction(options) {
	const proUpdaterPath = path.resolve(
		__dirname,
		"..",
		"..",
		"..",
	...
	const targetVersion =
		proSetup._testing &&
		typeof proSetup._testing.getProArtifactVersion === "function"
	...
	if (options.skipScaffold) {
		console.log(
			"Note: --skip-scaffold is not supported by the authenticated artifact updater.",
		);
	}
