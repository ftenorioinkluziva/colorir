...
function createValidateCommand() {
	...
	validate
		...
		.option(
			"--no-signature",
			"Skip manifest signature verification (insecure; recovery only)",
		)
		.option(
			"--require-signature",
...
function resolveSignatureRequirement({
	...
	if (
		...
		isTruthyEnv(env.AIOX_REQUIRE_SIGNATURE)
	) {
		return true;
	}

	const targetSignaturePath = path.join(
		aioxCoreDir,
	...
	if (sourceDir) {
		const sourceSignaturePath = path.join(
			sourceDir,
			".aiox-core",
			"install-manifest.yaml.minisig",
		);
