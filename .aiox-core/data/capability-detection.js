...
function detectDeferLoading() {
	...
	return {
		available: false,
		reason:
			"defer_loading is API-only (Python SDK). Not exposed in Claude Code CLI.",
...
function detectGlobalMcps() {
	...
	const dockerMcpConfigPath = path.join(
		os.homedir(),
		".docker",
		"mcp",
		"config.yaml",
...
function run() {
	...
	const capabilities = {
		...
		nonEssentialServers:
			toolRegistry.nonEssential.length > 0 ? toolRegistry.nonEssential : [],
	};
	...
	console.log(
		`   Tool Search: ${toolSearch.available ? "AVAILABLE" : "NOT AVAILABLE"}`,
	);
	console.log(
