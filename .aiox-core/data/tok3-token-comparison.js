...
for (const [name, wf] of Object.entries(workflows)) {
	...
	const reduction = (((directTotal - batchTotal) / directTotal) * 100).toFixed(
		1,
	);
...
const avgReduction = (((totalDirect - totalBatch) / totalDirect) * 100).toFixed(
	1,
);
...
console.log("");
console.log(
	"Note: These are estimated tokens based on tool-registry.yaml tokenCost",
);
console.log(
	"values and observed output sizes. True PTC (API-level) would yield ~37%",
);
console.log(
	"but is not available in Claude Code CLI (ADR-7). Bash batch achieves",
);
console.log(
	"~20-40% by consolidating tool calls and keeping intermediate results",
);
