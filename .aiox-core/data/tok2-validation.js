...
function run() {
	...
	check(1, "Runtime capability detection module exists", () => {
		return fs.existsSync(
			path.join(PROJECT_ROOT, ".aiox-core", "data", "capability-detection.js"),
		);
	});
	...
	check(4, "Tier 3 tools deferred via best available strategy", () => {
		const caps = JSON.parse(
			fs.readFileSync(
				path.join(PROJECT_ROOT, ".aiox", "runtime-capabilities.json"),
				"utf8",
			),
		);
		return (
	...
	check("5-6", "Tool Search latency and search limits documented", () => {
		...
		return (
			claudeMd.includes("2 searches per turn") ||
			claudeMd.includes("tool search")
		);
	});
	...
	check(9, "Essential MCP servers defined in tool-registry.yaml", () => {
		...
		return (
			registry.includes("essential: true") &&
			registry.includes("essential: false")
		);
	});
	...
	check(14, "Capability detection validates against actual MCPs", () => {
		...
		return (
			projectNames.includes("nogic") && projectNames.includes("code-graph")
		);
	});
	...
	check(17, "No functional regression (MCP workflows still work)", () => {
		...
		const mcpConfig = JSON.parse(
			fs.readFileSync(path.join(PROJECT_ROOT, ".mcp.json"), "utf8"),
		);
