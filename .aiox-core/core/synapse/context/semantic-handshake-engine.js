...
function normalizeType(value) {
	return Object.values(ConstraintType).includes(value)
		? value
		...
		.map((file, index) => {
			...
			return {
				...
				content: normalizeContent(
					file.content ?? file.after ?? file.diff ?? file.code,
				),
			};
		})
		.filter(Boolean);
...
const DEFAULT_EXTRACTORS = [
	...
	},
	{
		...
		constraint: {
			...
			severity: ConstraintSeverity.BLOCKER,
			description:
				"Use absolute imports; parent-directory relative imports are not allowed.",
...
class SemanticHandshakeEngine {
	...
	getConstraints() {
		return [...this._constraints.values()].map((constraint) =>
			this._publicConstraint(constraint),
		);
	}
	...
	generateComplianceReport(result) {
		...
		if (result.skippedConstraints?.length) {
			...
			for (const constraint of result.skippedConstraints) {
				lines.push(
					`- ${constraint.id}: not applicable to current code context`,
				);
			}
		}
		...
		if (result.violations?.length) {
			...
			for (const violation of result.violations) {
				lines.push(
					`- [${violation.severity}] ${violation.id}: ${violation.message}`,
				);
			}
		}
		...
		if (!result.passed) {
			lines.push("");
			lines.push(
				"Action: update the implementation or request an explicit architecture override.",
			);
		}
	...
	async _evaluateConstraint(constraint, codeContext, files, executionIntent) {
		...
		const forbiddenMatches = findPatternMatches(
	...
	_constraintApplies(constraint, codeContext) {
		...
		return constraint.appliesWhen.some((pattern) =>
			patternMatches(pattern, codeContext),
		);
	}
