...
class HierarchicalContextManager extends EventEmitter {
	...
	constructor(options = {}) {
		...
		this.maxTokens = isPositiveNumber(options.maxTokens)
			...
			: DEFAULT_MAX_TOKENS;
		this.summarizationThreshold = normalizeThreshold(
			options.summarizationThreshold,
		);
		this.thresholdTokens = Math.max(
			1,
			Math.floor(this.maxTokens * this.summarizationThreshold),
		);
		this.minRecentMessages =
			Number.isInteger(options.minRecentMessages) &&
			options.minRecentMessages >= 0
		...
		this._summarizer =
			typeof options.summarizer === "function" ? options.summarizer : null;
		this._onSwap = typeof options.onSwap === "function" ? options.onSwap : null;
		this._onSwapError =
			typeof options.onSwapError === "function" ? options.onSwapError : null;
	...
	getStats() {
		...
		return {
			...
			totalMessages:
				this._shortTermMessages.length + this._longTermSummaries.length,
			shortTermTokens,
			longTermTokens,
	...
	async _fitLongTermSummariesToBudget() {
		if (
			this._getTotalTokens() <= this.maxTokens ||
			this._longTermSummaries.length === 0
		) {
			return;
		}
	...
	_selectShortTermMessagesForSwap() {
		...
		const keepRecent =
			available > 1 ? Math.min(this.minRecentMessages, available - 1) : 0;
	...
	async _summarizeMessages(messages, options) {
		...
		if (this._summarizer) {
			try {
				rawSummary = await this._summarizer({
					...
					currentSummary: this._longTermSummaries
		...
		const content = this._truncateTextToTokenBudget(
			summaryContent,
			targetTokens,
		);
	...
	_truncateSummaryMessage(message) {
		...
		cloned.content = this._truncateTextToTokenBudget(
			cloned.content,
			this._summaryTargetTokens(),
		);
		cloned.metadata = {
			...(cloned.metadata || {}),
			aiox: {
				...(cloned.metadata && cloned.metadata.aiox
					? cloned.metadata.aiox
					: {}),
	...
	_truncateTextToTokenBudget(text, tokenBudget) {
		...
		while (
			...
			const nextLength = Math.max(
				0,
				Math.floor(result.length * ratio) - suffix.length - 8,
			);
	...
	_getTotalTokens() {
		return this._countMessagesTokens([
			...this._longTermSummaries,
			...this._shortTermMessages,
		]);
	}
	...
	_countMessagesTokens(messages) {
		return messages.reduce(
			(total, message) => total + this._countMessageTokens(message),
			0,
		);
	}
	...
	_countMessageTokens(message) {
		...
		const contentTokens = this._countTextTokens(
	...
	_countTextTokens(text) {
		...
		if (this._tokenizer && typeof this._tokenizer.countTokens === "function") {
			const result = normalizeTokenCount(
				this._tokenizer.countTokens(normalizedText),
			);
			if (result !== null) return result;
		}
		...
		if (this._tokenizer && typeof this._tokenizer.encode === "function") {
			const result = normalizeTokenCount(
				this._tokenizer.encode(normalizedText),
			);
			if (result !== null) return result;
		}
