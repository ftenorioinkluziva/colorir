...
try {
	...
	if (
		!gotchasMemoryModule ||
		typeof gotchasMemoryModule.GotchasMemory === "undefined"
	) {
		throw new Error(
			`Missing named export GotchasMemory from ${GOTCHAS_MEMORY_MODULE}`,
		);
	}
	...
	executeClaude(prompt) {
		if (!prompt || typeof prompt !== "string") {
			return Promise.reject(
				new Error("executeClaude requires a non-empty string prompt"),
			);
		}
		...
		return new Promise((resolve, reject) => {
			...
			try {
				...
				child.kill?.();
				rejectOnce(
					new Error(`Claude CLI stdin write failed: ${error.message}`),
				);
				return;
			}
			...
			child.on("close", (code, signal) => {
				if (stdinError) {
					rejectOnce(
						new Error(`Claude CLI stdin write failed: ${stdinError.message}`),
					);
				} else if (code === 0) {
					resolveOnce({
						success: true,
					...
					rejectOnce(
						new Error(
							`Claude CLI killed by signal ${signal}: ${stderr || stdout}`,
						),
					);
				} else {
					rejectOnce(
