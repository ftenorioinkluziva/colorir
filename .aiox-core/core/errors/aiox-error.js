...
class AIOXError extends Error {
	constructor(message, options = {}) {
		...
		const finalMessage =
			message || options.message || definition.userMessage || code;
...
function isAIOXError(value) {
	return (
		value instanceof AIOXError || Boolean(value && value.isAIOXError === true)
	);
}
...
function normalizeError(error, overrides = {}) {
	if (isAIOXError(error)) {
		...
		return new AIOXError(overrides.message || error.message, {
			...
			retryable: hasOwn(overrides, "retryable")
				? overrides.retryable
				: error.retryable,
			exitCode: hasOwn(overrides, "exitCode")
				? overrides.exitCode
				: error.exitCode,
	...
	if (error instanceof Error) {
		...
		const metadata = deepMerge(
			...
			Object.keys(ownProperties).length > 0
				? { originalError: { properties: ownProperties } }
				: {},
	...
	return new AIOXError(overrides.message || String(error), {
		...
		recovery: overrides.recovery,
		metadata: deepMerge(
			{ originalValue: { type: typeof error } },
			overrides.metadata || {},
		),
