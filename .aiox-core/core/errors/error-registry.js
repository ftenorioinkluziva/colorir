...
function createUnknownDefinition(code = DEFAULT_ERROR_CODE) {
	return freezeDefinition({
		...
		metadata:
			code === DEFAULT_ERROR_CODE ? {} : { registry: { registered: false } },
	});
}
...
class ErrorRegistry {
	...
	lookup(code) {
		...
		const fallback =
			this._entries.get(DEFAULT_ERROR_CODE) || createUnknownDefinition();
		return freezeDefinition({
			...fallback,
			code: normalizedCode,
			metadata: deepMerge(fallback.metadata, {
	...
	_normalizeDefinition(definition) {
		...
		const normalized = {
			...
			metadata: isPlainObject(definition.metadata)
				? cloneMetadataValue(definition.metadata)
				: {},
		};
