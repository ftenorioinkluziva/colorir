...
function acquireFileLock(filePath, options = {}) {
	...
	const attempts =
		options.lockRetryAttempts ?? DEFAULT_CONFIG.lockRetryAttempts;
...
function diagnoseCause(error) {
	...
	const text =
		`${normalized.name} ${normalized.message} ${normalized.stack}`.toLowerCase();

	if (
		/\b(context|token|window|length|too large|max tokens|overflow)\b/.test(text)
	) {
		return CauseOfDeath.CONTEXT_OVERFLOW;
	}
	if (
		/\b(recursive|recursion|loop|same action|repeated action|maximum call stack)\b/.test(
			text,
		)
	) {
	...
	if (
		/\b(tool|function call|schema|invalid arguments|command failed|spawn|enoent)\b/.test(
			text,
		)
	) {
	...
	if (
		/\b(api|network|auth|unauthorized|forbidden|timeout|econn|http|rate limit)\b/.test(
			text,
		)
	) {
...
function normalizeState(state = {}) {
	...
	return {
		...
		workingMemory: Array.isArray(state.workingMemory)
			? state.workingMemory
			: [],
		criticalVariables:
		...
		currentAction:
			state.currentAction || state.current_action || state.action || "",
...
function buildLegacy(state, cause, options = {}) {
	...
	const omittedMemoryItems = Math.max(
		0,
		normalized.workingMemory.length - memoryTail.length,
	);
	...
	return {
		...
		summary: truncate(
			summary,
			options.maxLegacyChars ?? DEFAULT_CONFIG.maxLegacyChars,
		),
	};
}
...
function generateDirectives(cause, error, state = {}) {
	...
	if (normalizedState.currentAction) {
		directives.push(
			`Do not immediately repeat action: ${normalizedState.currentAction}`,
		);
	}
	...
	switch (cause) {
		case CauseOfDeath.CONTEXT_OVERFLOW:
			...
			directives.push(
				"Summarize any large memory block before adding it to active context.",
			);
			break;
		case CauseOfDeath.TOOL_EXECUTION_FAILURE:
			directives.push(
				"Validate tool parameters against the schema before retrying.",
			);
			directives.push(
		...
		case CauseOfDeath.RECURSIVE_LOOP:
			...
			directives.push(
				"After two similar failures, pivot strategy or escalate.",
			);
			break;
		case CauseOfDeath.EXTERNAL_API_ERROR:
			directives.push(
				"Check credentials, rate limits and network availability before retrying.",
			);
			directives.push(
				"Prefer a cached/offline fallback if the external dependency is unstable.",
			);
			break;
		default:
			directives.push(
				"Read the autopsy report and proceed with an explicit recovery plan.",
			);
	}
...
function buildReincarnationContext(report) {
	return [
		...
		...report.preventionDirectives.map(
			(directive, index) => `${index + 1}. ${directive}`,
		),
		"",
		"Immunity token:",
...
class StateCommitLog {
	...
	list(agentId = null) {
		...
		return agentId
			? commits.filter((commit) => commit.agentId === agentId)
			: commits;
	}
...
class EvolutionLog {
	...
	getToxicActions(events = this.list()) {
		return events.reduce((actions, event) => {
			...
			actions[event.forbiddenAction] =
				(actions[event.forbiddenAction] || 0) + 1;
			return actions;
		}, {});
	}
}
	...
	claimNext() {
		return withFileLock(this.filePath, this.config, () => {
			...
			const index = queue.findIndex(
				(item) => item.status === QueueStatus.QUEUED,
			);
...
class AutopsyEngine extends EventEmitter {
	constructor(projectRoot = process.cwd(), options = {}) {
		...
		this.filePath = path.join(
			this.projectRoot,
			this.config.dataDir,
			this.config.autopsyFile,
		);
		this.stateCommitLog =
			options.stateCommitLog || new StateCommitLog(projectRoot, this.config);
	}
	...
	recordDeath(state, error, metadata = {}) {
		...
		const { legacy, summary } = buildLegacy(
			normalizedState,
			cause,
			this.config,
		);
		const preventionDirectives = generateDirectives(
			cause,
			error,
			normalizedState,
		);
...
class AgentImmortalityProtocol extends EventEmitter {
	constructor(projectRoot = process.cwd(), options = {}) {
		...
		this.reincarnationQueue =
			options.reincarnationQueue ||
			new ReincarnationQueue(projectRoot, this.config);
		this.evolutionLog =
			options.evolutionLog || new EvolutionLog(projectRoot, this.config);
	...
	_trackGotcha(report) {
		if (
			!this.gotchasMemory ||
			typeof this.gotchasMemory.trackError !== "function"
		) {
			return;
		}
