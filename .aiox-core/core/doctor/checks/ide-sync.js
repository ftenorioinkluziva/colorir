...
function readMarkdownAgents(dir, label) {
	...
	try {
		...
		return {
			agents: [],
			error: `Cannot read ${label} directory: ${error.message}`,
		};
	}
}
...
function readSkillAgents(dir) {
	...
	try {
		...
		return {
			agents: [],
			error: `Cannot read Claude skills directory: ${error.message}`,
		};
	}
}
...
async function run(context) {
	...
	const agentsCommandDir = path.join(
		context.projectRoot,
		".claude",
		"commands",
		"AIOX",
	...
	const agentsSkillDir = path.join(
		context.projectRoot,
		".claude",
		"skills",
		"AIOX",
