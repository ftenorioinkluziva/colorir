...
const CORE_PACKAGE_REQUIRE = createRequire(
	path.join(CORE_PACKAGE_ROOT, "package.json"),
);
...
function assertValidProjectRoot(projectRoot) {
	if (!projectRoot || typeof projectRoot !== "string") {
		throw new TypeError(
			"updatePro(projectRoot): projectRoot must be a non-empty string.",
		);
	}
...
function getCoreVersion(projectRoot) {
	...
	for (const packageName of CORE_PACKAGES) {
		const packageJsonPath = buildNodeModulesPackageJsonPath(
			resolvedProjectRoot,
			packageName,
		);
		if (fs.existsSync(packageJsonPath)) {
		...
		if (declaredCorePackage) {
			for (const field of DEPENDENCY_FIELDS) {
				const declaredVersion =
					projectPackageJson[field]?.[declaredCorePackage];
...
async function applyScaffoldStep(
	...
	try {
		...
		result.actions.push({
			action: "scaffold",
			status: scaffoldResult.success ? "done" : "failed",
		});
...
async function updatePro(projectRoot, options = {}) {
	...
	if (
		requiredCore &&
		coreVersion &&
		!satisfiesPeer(coreVersion, requiredCore)
	) {
	...
	if (dryRun) {
		...
		if (includeCoreUpdate) {
			const corePackageName =
				detectCorePackageName(resolvedProjectRoot) || "aiox-core";
			result.actions.push({
				action: "core_update",
				status: "dry_run",
	...
	onProgress(
		"update",
		`Updating ${installed.packageName} to ${latest.version}...`,
	);
	try {
