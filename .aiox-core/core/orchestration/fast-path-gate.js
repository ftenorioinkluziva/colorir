...
const STRUCTURED_FILE_EXTENSION_SET = new Set(
	STRUCTURED_FILE_EXTENSIONS_FROZEN,
);
	...
	[
		{
			...
			weight: 3,
			pattern:
				/\b(batch|bulk|many files|multiple files|all files|in one shot|one shot)\b/i,
		},
		{
			id: "structured-transform",
			weight: 3,
			pattern:
				/\b(yaml|json|csv|markdown|frontmatter|schema|variable|variables|field|fields)\b/i,
		},
		{
			id: "mechanical-edit",
			weight: 3,
		...
		},
		{
			...
			weight: 2,
			pattern:
				/\b(repeated|repetitive|same change|similar change|dumb task|tedious)\b/i,
		},
		{
			id: "parallelizable",
			weight: 2,
...
const RISK_PATTERNS_FROZEN = Object.freeze(
	[
		...
		},
		{
			...
			weight: 3,
			pattern:
				/\b(security|secret|token|credential|auth|permission|pii|rls)\b/i,
		},
		{
			id: "destructive",
			weight: 3,
			pattern:
				/\b(delete|remove data|drop|reset|rewrite history|destructive)\b/i,
		},
		{
			id: "production",
			weight: 2,
...
function clonePatternDefinition(patternDefinition) {
	return {
		...
		pattern: new RegExp(
			patternDefinition.pattern.source,
			patternDefinition.pattern.flags,
		),
	};
}
...
function scoreFastPath({
	...
	const automationWeight = automationSignals.reduce(
		(sum, signal) => sum + signal.weight,
		0,
	);
	const riskWeight = riskSignals.reduce(
		(sum, signal) => sum + signal.weight,
		0,
	);
...
function chooseMode({
	...
	if (
		externalExecutorsEnabled &&
		confidence >= config.externalExecutorThreshold
	) {
...
function evaluateFastPath(input = {}) {
	...
	const batchSize =
		task.itemCount ?? Math.max(task.files.length, structuredFileCount);
